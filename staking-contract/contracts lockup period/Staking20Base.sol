...
contract Staking20Base is ContractMetadata, Multicall, Ownable, Staking20 {
    ...
    /// @dev Mapping of user to lockup end time.
    mapping(address => uint256) public userLockupEndTime;
    ...
    /// @dev Staking function with additional checks for max staked tokens per user, total, and lockup period.
    function _stake(uint256 _amount) internal override {
        ...
        // Set the lockup end time
        userLockupEndTime[msg.sender] = block.timestamp + lockupDuration;

        super._stake(_amount);
    }

    /// @dev Withdraw function with additional checks for staked tokens and lockup period.
    function _withdraw(uint256 _amount) internal override {
        require(userStakedTokens[msg.sender] >= _amount, "Insufficient staked tokens");
        require(block.timestamp >= userLockupEndTime[msg.sender], "Tokens are still in lockup period");
